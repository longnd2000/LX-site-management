...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFields) => {
    ...
    try {
      ...
      setErrorMsg(translateAuthError(err.message || 'Email hoặc mật khẩu không chính xác.'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden font-sans text-white">
      ...
      <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-md border border-slate-800/60 p-8 rounded-2xl shadow-2xl z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white m-0">LX Site Management</h2>
          <p className="text-slate-400 text-sm mt-2">Đăng nhập để quản trị các website vệ tinh</p>
        </div>
